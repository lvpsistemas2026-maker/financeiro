...
import { Plus, Trash2, Edit2, Filter, ChevronDown, X, Loader2, TrendingUp, CheckCircle } from 'lucide-react'
...
export default function RecebimentosClient({ recebimentos: initial, lojas, categorias }: Props) {
  ...
  const openNovo = () => {
    setEditando(null)
    setForm({ loja_id: '', categoria_id: '', descricao: '', valor: '', data_recebimento: new Date().toISOString().split('T')[0], status: 'pendente', forma_recebimento: '', observacao: '' })
    setShowModal(true)
  }

  const openEditar = (r: any) => {
    setEditando(r)
    setForm({ loja_id: String(r.loja_id ?? ''), categoria_id: String(r.categoria_id ?? ''), descricao: r.descricao, valor: String(r.valor), data_recebimento: r.data_recebimento, status: r.status, forma_recebimento: r.forma_recebimento ?? '', observacao: r.observacao ?? '' })
    setShowModal(true)
  }

  const handleSalvar = () => {
    if (!form.descricao || !form.valor || !form.data_recebimento) { toast.error('Preencha os campos obrigatórios'); return }
    startTransition(async () => {
      try {
        const data = {
          ...
          data_recebimento: form.data_recebimento, status: form.status as any,
          forma_recebimento: form.forma_recebimento || undefined, observacao: form.observacao || undefined,
        }
        if (editando) { await updateRecebimento(editando.id, data); toast.success('Recebimento atualizado!') }
        else { await createRecebimento(data); toast.success('Recebimento criado!') }
  ...
  const handleDeletar = (id: number) => {
    ...
    startTransition(async () => { try { await deleteRecebimento(id); toast.success('Excluído!'); router.refresh() } catch { toast.error('Erro') } })
  }

  const handleMarcarRecebido = (r: any) => {
    startTransition(async () => { try { await updateRecebimento(r.id, { status: 'recebido' }); toast.success('Marcado como recebido!'); router.refresh() } catch { toast.error('Erro') } })
  }
          ...
          <div className="relative bg-card border border-border rounded-xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={form.descricao} onChange={e => setForm(f => ({ ...f, descricao: e.target.value }))} placeholder="Descrição do recebimento"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="block text-xs font-medium text-muted-foreground mb-1.5">Forma</label><Sel value={form.forma_recebimento} onChange={v => setForm(f => ({ ...f, forma_recebimento: v }))} placeholder="Selecione">{FORMAS.map(f => <option key={f} value={f}>{f}</option>)}</Sel></div>
              </div>
              <div>
